...
# ==========================================
# 5. Main Content: Tabs Layout (Reduced to 3 Tabs)
# ==========================================
...
# ------------------------------------------
# TAB 1: Data Exploration (Re-ordered Layout)
# ------------------------------------------
with tab_eda:
    ...
    # --- 4. EXPLORATORY VISUALIZATIONS (Moved Down & Shown Directly in 2 columns) ---
    ...
    # Automatically plot all distribution graphs in 2 columns
    ...
    st.markdown("##### 📌 Feature vs Feature Analysis")
    
    figures_to_plot = []

    # 1. Level vs Achievements
    fig1, ax1 = plt.subplots(figsize=(8, 5))
    sns.scatterplot(data=df, x='PlayerLevel', y='AchievementsUnlocked', hue='EngagementLevel',
                    palette={'Low':'#99ff99', 'Medium':'#ff9999', 'High':'#66b3ff'}, alpha=0.7, ax=ax1)
    ax1.set_title('Player Level vs. Achievements Unlocked', fontsize=14, weight='bold')
    sns.despine()
    figures_to_plot.append(fig1)

    # 2. In-Game Purchase by Genre
    fig2, ax2 = plt.subplots(figsize=(8, 5))
    genre_purchase = df.groupby('GameGenre')['InGamePurchases'].mean().sort_values().reset_index()
    sns.barplot(data=genre_purchase, x='GameGenre', y='InGamePurchases', palette='mako', ax=ax2)
    ax2.set_title('In-Game Purchase Rate by Game Genre', fontsize=14, weight='bold')
    sns.despine()
    figures_to_plot.append(fig2)

    # 3. Session Duration Density by Difficulty
    fig3, ax3 = plt.subplots(figsize=(8, 5))
    sns.kdeplot(data=df, x='AvgSessionDurationMinutes', hue='GameDifficulty', fill=True, alpha=0.5,
                palette={'Easy':'#3498db', 'Medium':'#e74c3c', 'Hard':'#2ecc71'}, ax=ax3)
    ax3.set_title('Session Duration Density by Game Difficulty', fontsize=14, weight='bold')
    sns.despine()
    figures_to_plot.append(fig3)

    # 4. Game Genre by Gender
    fig4, ax4 = plt.subplots(figsize=(8, 5))
    sns.countplot(data=df, x='GameGenre', hue='Gender', palette='Set2', ax=ax4)
    ax4.set_title('Game Genre Preferences by Gender', fontsize=14, weight='bold')
    sns.despine()
    figures_to_plot.append(fig4)
    ...
    sns.countplot(data=df, x='Location', hue='EngagementLevel', 
                  ...
                  palette=['#ff9999','#66b3ff','#99ff99'], ax=ax5)
    ax5.set_title('Player Engagement Levels by Location', fontsize=14, weight='bold')
    sns.despine()
    figures_to_plot.append(fig5)

    # 6. Weekly Sessions Based on Game Difficulty
    fig6, ax6 = plt.subplots(figsize=(8, 5))
    sns.boxplot(data=df, x='GameDifficulty', y='SessionsPerWeek', 
                order=['Easy', 'Medium', 'Hard'], palette='Wistia', ax=ax6)
    ax6.set_title('Weekly Sessions Based on Game Difficulty', fontsize=14, weight='bold')
    sns.despine()
    figures_to_plot.append(fig6)

    # Automatically plot all relationship graphs in 2 columns
    ...
    # ========================================================
    # 6. ABOUT THE SYSTEM (Moved from Tab 4, Simplified & Styled)
    # ========================================================
    st.markdown("---")
    ...
    about_html = (
        ...
        'engaged a player really is. Built end-to-end: exploring raw data, engineering features, '
        'comparing models, and generating live predictions.</p>'
        '<div style="display: flex; gap: 15px; margin-top: 20px;">'
        '<div style="background: rgba(255,255,255,0.2); padding: 15px; border-radius: 8px; flex: 1;">'
        '<strong>📊 EDA & Features</strong><br>'
        '<span style="font-size: 13px;">Explores relationships & engineered features like <i>TotalWeeklyMinutes</i>.</span>'
        '</div>'
        '<div style="background: rgba(255,255,255,0.2); padding: 15px; border-radius: 8px; flex: 1;">'
        '<strong>🤖 Best Model</strong><br>'
        '<span style="font-size: 13px;"><b>XGBoost</b> outperforms others, achieving a <b>96.9% Test Accuracy</b>.</span>'
        '</div>'
        '<div style="background: rgba(255,255,255,0.2); padding: 15px; border-radius: 8px; flex: 1;">'
        '<strong>⚡ Live Predictor</strong><br>'
        '<span style="font-size: 13px;">Instantly evaluates profiles & provides actionable retention strategies.</span>'
        '</div>'
        '</div>'
        '</div>'
    )
...
with tab_perf:
    ...
    for i, model_name in enumerate(performance_models):
        with btn_cols[i]:
            button_label = f"✓ {model_name}" if current_perf_model == model_name else model_name
    ...
    classification_reports = {
        "Logistic Regression": {
            "Low": {"precision": 0.89, "recall": 0.90, "f1-score": 0.90, "support": 2065},
            "Medium": {"precision": 0.89, "recall": 0.92, "f1-score": 0.90, "support": 3875},
            "High": {"precision": 0.95, "recall": 0.88, "f1-score": 0.91, "support": 2067},
            "macro avg": {"precision": 0.91, "recall": 0.90, "f1-score": 0.90, "support": 8007},
            "weighted avg": {"precision": 0.91, "recall": 0.90, "f1-score": 0.90, "support": 8007},
            "accuracy": 0.9040
        },
        "Random Forest": {
            "Low": {"precision": 0.95, "recall": 0.96, "f1-score": 0.96, "support": 2065},
            "Medium": {"precision": 0.94, "recall": 0.96, "f1-score": 0.95, "support": 3875},
            "High": {"precision": 0.97, "recall": 0.92, "f1-score": 0.94, "support": 2067},
            "macro avg": {"precision": 0.96, "recall": 0.95, "f1-score": 0.95, "support": 8007},
            "weighted avg": {"precision": 0.95, "recall": 0.95, "f1-score": 0.95, "support": 8007},
            "accuracy": 0.9510
        },
        "KNN": {
            "Low": {"precision": 0.93, "recall": 0.71, "f1-score": 0.80, "support": 2065},
            "Medium": {"precision": 0.78, "recall": 0.96, "f1-score": 0.86, "support": 3875},
            "High": {"precision": 0.96, "recall": 0.78, "f1-score": 0.86, "support": 2067},
            "macro avg": {"precision": 0.89, "recall": 0.81, "f1-score": 0.84, "support": 8007},
            "weighted avg": {"precision": 0.86, "recall": 0.85, "f1-score": 0.84, "support": 8007},
            "accuracy": 0.8461
        },
        "XGBoost": {
            "Low": {"precision": 0.97, "recall": 0.98, "f1-score": 0.97, "support": 2065},
            "Medium": {"precision": 0.96, "recall": 0.97, "f1-score": 0.97, "support": 3875},
            "High": {"precision": 0.98, "recall": 0.95, "f1-score": 0.97, "support": 2067},
            "macro avg": {"precision": 0.97, "recall": 0.97, "f1-score": 0.97, "support": 8007},
            "weighted avg": {"precision": 0.97, "recall": 0.97, "f1-score": 0.97, "support": 8007},
            "accuracy": 0.9694
        }
    }
...
with tab_pred:
    ...
    if predict_btn:
        ...
        with col_prob:
            ...
            fig_prob = px.bar(prob_df, x="Probability", y="Engagement Level", orientation='h', text_auto='.1%', 
                              color="Engagement Level", color_discrete_map={'Low': '#ff9999', 'Medium': '#66b3ff', 'High': '#99ff99'})
            fig_prob.update_layout(xaxis=dict(range=[0, 1], tickformat=".0%"), showlegend=False, height=250, margin=dict(l=0, r=0, t=30, b=0))
